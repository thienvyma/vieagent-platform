# Comprehensive VIEAgent Environment Setup
# Complete script covering all environment variables used in the project
...
Write-Host "VIEAgent Comprehensive Environment Setup" -ForegroundColor Cyan
...
# =============================================================================
Write-Host "`n=== CHROMADB CONFIGURATION ===" -ForegroundColor Yellow
...
$useEmail = Read-Host "Configure SMTP Email? (y/N)" "N"
...
if ($useStripe -eq "y" -or $useStripe -eq "Y") {
    $stripePublicKey = Read-Host "Enter STRIPE_PUBLIC_KEY (pk_test_...)"
    $stripeSecretKey = Read-Host "Enter STRIPE_SECRET_KEY (sk_test_...)"
...
# ChromaDB
if ($useChromaDB -eq "y" -or $useChromaDB -eq "Y") {
    $replacements["localhost"] = $chromaHost
    $replacements["8000"] = $chromaPort
}
...
if ($useChromaDB -eq "y" -or $useChromaDB -eq "Y") { Write-Host "- Vector Database: ChromaDB" -ForegroundColor White }
if ($useEmail -eq "y" -or $useEmail -eq "Y") { Write-Host "- Email: SMTP" -ForegroundColor White }
